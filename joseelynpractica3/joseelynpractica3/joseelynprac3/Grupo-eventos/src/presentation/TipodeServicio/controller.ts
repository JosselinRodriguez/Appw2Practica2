...
export class TpservicioController {
  ...
  public deletetiservicio = async (req: Request, res: Response) => {
    const id = +req.params.id;
    const tipoServicio = await prisma.tipoServicio.findFirst({
      where: { id }
    });

    if (!tipoServicio) return res.status(404).json({ error: `tipoServicio with id${id} not found` });
    const deleted = await prisma.tipoServicio.delete({
      where: { id }
    });
    (deleted)
      ? res.json(deleted)
      : res.status(400).json({ error: `tipoServicio with id ${id} not found` });
  };
}